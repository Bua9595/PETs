"""Re-extract Ruffy Gear3 v2 sprite candidates from the new gear3 source sheet.

Reads ONLY ``assets/source/pet1_ruffy_gear3_source_v2.png``. Does not use archived
source_cells, row_sources, runtime atlases, review sheets, codex atlas, the old
gear3 raw sheet, or normal/gear2 approved sprites.
...
Usage::
    ...
    python scripts/reextract_ruffy_gear3_v2_from_raw.py
"""
...
RAW_SOURCE = REPO_ROOT / "assets" / "source" / "pet1_ruffy_gear3_source_v2.png"

# New controlled Gear3 source sheet: 4 columns x 2 rows (8 cells).
GRID_COLS = 4
GRID_ROWS = 2
...
CANDIDATES_DIR = REPO_ROOT / "assets" / "review" / "candidates" / "ruffy_gear3_reextract_v2"
CSV_PATH = REPO_ROOT / "assets" / "review" / "reports" / "ruffy_gear3_reextract_v2_review.csv"
CONTACT_SHEET = REPO_ROOT / "assets" / "review" / "contact_sheets" / "ruffy_gear3_reextract_v2_review.png"
DIAGNOSIS_PATH = REPO_ROOT / "assets" / "review" / "reports" / "ruffy_gear3_reextract_v2_diagnosis.md"
...
def assess_gear3_form(rgba: Image.Image) -> tuple[str, str, list[str]]:
    bbox = content_bbox(rgba)
    if bbox is None:
        return "unclear", "no_visible_content", ["empty after chroma key"]
    ...
    if not opaque:
        return "unclear", "no_visible_content", ["no opaque pixels"]
    ...
    if head_opaque > 40 and white_head / head_opaque > 0.18:
        ...
        return "unclear", "gear5_white_hair_or_clothing", notes
    ...
    if dark_haki / len(opaque) > 0.34:
        ...
        return "unclear", "gear4_haki_arms", notes

    content_w = right - left
    content_h = bottom - top
    if content_w > content_h * 1.18 or content_w > w * 0.7:
...
def extract_cell(cell_rgb: Image.Image, grid_row: int) -> tuple[Image.Image | None, bool, bool, str]:
    """Extract one cell with generous symmetric padding around content bbox."""
    cw, ch = cell_rgb.size
...
def decide_status(
    ...
    """Only pending or needs_fix — never auto-approve or auto-reject."""
    if empty:
        return "needs_fix", "no_visible_content"
    ...
    if looks_like == "unclear" or wrong_reason:
        return "needs_fix", wrong_reason or "gear3_form_unclear"
    return "pending", ""
...
def build_contact_sheet(candidates: list[Candidate]) -> None:
    ...
    draw.text(
        (pad, 18),
        "ruffy gear3 re-extract v2 | REVIEW ONLY - raw source extraction - not approved",
        fill=(235, 235, 240),
        font=title_font,
    )
...
def write_diagnosis(raw_size: tuple[int, int], raw_mode: str, candidates: list[Candidate]) -> None:
    ...
    wrong = sum(1 for c in candidates if c.looks_like_gear3 == "unclear" and c.wrong_form_reason)

    lines = [
        "# Ruffy Gear3 Re-Extraction v2 Diagnosis",
        "",
        "## Raw Source",
        "",
...
def main() -> int:
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS):
            ...
            cid = f"ruffy_gear3_reextract_v2_{index:04d}"
            rel_file = f"assets/review/candidates/ruffy_gear3_reextract_v2/{cid}.png"
            ...
            if extracted is None:
                ...
                candidates.append(
                    Candidate(
                        candidate_id=cid,
                        file_path=rel_file,
                        source_path=RAW_SOURCE.relative_to(REPO_ROOT).as_posix(),
                        width=0,
                        height=0,
                        top_margin=0,
                        bottom_margin=0,
    ...
    needs_fix = sum(1 for c in candidates if c.review_status == "needs_fix")
    cutoff = sum(1 for c in candidates if c.cutoff_risk)
    print(f"candidates: {len(candidates)} pending={pending} needs_fix={needs_fix} cutoff_risk={cutoff}")
