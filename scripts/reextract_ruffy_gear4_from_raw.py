"""Re-extract Ruffy Gear4 sprite candidates from the raw gear4 source sheet.

Reads ONLY ``assets/source/pet1_ruffy_gear4_source.png``. Does not use archived
source_cells, row_sources, runtime atlases, review sheets, codex atlas, or
normal/gear2/gear3 approved sprites.

Outputs review artefacts under ``assets/review/`` — nothing is approved and
nothing is copied into ``assets/source_approved/ruffy/gear4/``.

Usage::

    python scripts/reextract_ruffy_gear4_from_raw.py
"""
...
RAW_SOURCE = REPO_ROOT / "assets" / "source" / "pet1_ruffy_gear4_source.png"

# Gear4 raw sheet layout (see src/pet1_ruffy_builder.py SOURCES["gear4"]).
GRID_COLS = 5
GRID_ROWS = 4
...
CANDIDATES_DIR = REPO_ROOT / "assets" / "review" / "candidates" / "ruffy_gear4_reextract"
CSV_PATH = REPO_ROOT / "assets" / "review" / "reports" / "ruffy_gear4_reextract_review.csv"
CONTACT_SHEET = REPO_ROOT / "assets" / "review" / "contact_sheets" / "ruffy_gear4_reextract_review.png"
DIAGNOSIS_PATH = REPO_ROOT / "assets" / "review" / "reports" / "ruffy_gear4_reextract_diagnosis.md"
...
def assess_gear4_form(rgba: Image.Image) -> tuple[str, str, list[str]]:
    """Heuristic Gear4 vs wrong-form check. Conservative — manual review preferred."""
    bbox = content_bbox(rgba)
    if bbox is None:
    ...
    # Gear3 signal: extremely wide content bbox (giant single limb dominates).
    ...
    # Gear2 signal: heavy pink/white steam vapor without dark haki arms.
    steam_like = 0
    dark_haki = 0
    for x, y in opaque:
        r, g, b, _ = pixels[x, y]
        ...
        if r < 55 and g < 45 and b < 45:
            ...
            dark_haki += 1
        elif r < 80 and g < 70 and b < 90 and r - g > 15:
            dark_haki += 1
    ...
    if steam_like > 80 and haki_ratio < 0.06:
        notes.append(f"steam-like pixels {steam_like} without haki signal")
        return "false", "gear2_steam_only", notes

    # Bulkier/darker body tones can still be valid Gear4 without extreme haki.
    mid_tone_dark = sum(
        1
        for x, y in opaque
        if pixels[x, y][0] < 120 and pixels[x, y][1] < 100 and pixels[x, y][2] < 120
    )
    if mid_tone_dark / len(opaque) > 0.22:
        notes.append(f"bulk/dark body tone ratio {mid_tone_dark}/{len(opaque)}")
        return "true", "", notes

    notes.append("no strong Gear4 haki/bulk signal; manual review recommended")
...
def decide_status(
    ...
    if looks_like == "false" and wrong_reason == "gear2_steam_only":
    ...
    if looks_like == "unclear":
        return "needs_fix", "gear4_form_unclear"
    return "pending", ""
...
def build_contact_sheet(candidates: list[Candidate]) -> None:
    ...
    draw.text(
        (pad, 18),
        "ruffy gear4 re-extract | REVIEW ONLY - raw source extraction - not approved",
        fill=(235, 235, 240),
        font=title_font,
    )
...
def write_diagnosis(raw_size: tuple[int, int], raw_mode: str, candidates: list[Candidate]) -> None:
    ...
    wrong = sum(1 for c in candidates if c.looks_like_gear4 == "false")

    lines = [
        "# Ruffy Gear4 Re-Extraction Diagnosis",
        "",
        "## Raw Source",
        "",
        ...
        "* gleiche Grundfigur",
        "* klar Gear4-Form",
        "* kraeftigere/bulkigere Form erlaubt",
        "* dunklere/kraeftige Arme moeglich",
        "* kein Gear2-only",
        "* kein Gear3-only",
        "* kein Gear5-Look",
        "* keine weissen Haare",
...
def main() -> int:
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS):
            ...
            cid = f"ruffy_gear4_reextract_{index:04d}"
            rel_file = f"assets/review/candidates/ruffy_gear4_reextract/{cid}.png"
            ...
            looks, wrong, form_notes = assess_gear4_form(extracted)
            all_notes = "; ".join(filter(None, [extract_notes, *form_notes]))
            status, reject = decide_status(cutoff_risk, source_clipped, looks, wrong, empty=False)
    ...
    rejected = sum(1 for c in candidates if c.review_status == "rejected")
    cutoff = sum(1 for c in candidates if c.cutoff_risk)
    print(f"candidates: {len(candidates)} pending={pending} needs_fix={needs_fix} rejected={rejected} cutoff={cutoff}")
